...
std::string_view ArgParser::GetArgName(const std::string_view& argument, bool& result) {
    ...
    size_t pos_def = argument.find("--") != std::string::npos ? argument.find("--") + 1 : argument.find('-');
...
std::vector<std::string> ArgParser::GetStringMultiValue(const std::string& name_of_param) {
    const StringArgument* argument = FindStringArgument(name_of_param);

    return argument->GetMultiValue();
}
...
void ArgParser::CheckArguments(const std::vector<std::string> &argv, bool& result, bool& is_help) {
    ...
    for (const auto& argument : argv) {
        ...
        if (string_argument) {
            ...
            for (const auto& short_pseud : arg_name) {
                flag = FindFlag(std::string(1, short_pseud));
                if (flag) {
                    flag->SetValue();
                } else {
                    std::cerr << std::format("Flag with short pseudonym {} is not exists.\n",
                                             short_pseud);
                    result = false;
                    return;
                }
